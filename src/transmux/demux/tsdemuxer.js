...
class TSDemuxer {
  ...
  append (data, timeOffset, contiguous, accurateTimeOffset) {
    ...
    for (start = syncOffset; start < len; start += 188) {
      if (data[start] === 0x47) {
        ...
        this.observer.trigger(Event.ERROR, { type: ErrorType.MEDIA_ERROR, details: ErrorDetail.FRAG_PARSING_ERROR, fatal: false, reason: 'TS packet did not start with 0x47' });
      }
    }
  ...
  _parseAACPES (pes) {
    ...
    if (offset) {
      ...
      this.observer.trigger(Event.ERROR, { type: ErrorType.MEDIA_ERROR, details: ErrorDetail.FRAG_PARSING_ERROR, fatal: fatal, reason: reason });
      if (fatal) {
        return;
      }
    }
