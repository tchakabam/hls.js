...
class MP4Remuxer {
  ...
  generateIS (audioTrack, videoTrack, timeOffset) {
    ...
    if (Object.keys(tracks).length) {
      ...
      observer.trigger(Event.ERROR, { type: ErrorType.MEDIA_ERROR, details: ErrorDetail.FRAG_PARSING_ERROR, fatal: false, reason: 'no audio/video samples found' });
    }
  }
  ...
  remuxVideo (track, timeOffset, contiguous, audioTrackLength, accurateTimeOffset) {
    ...
    try {
      ...
      this.observer.trigger(Event.ERROR, { type: ErrorType.MUX_ERROR, details: ErrorDetail.REMUX_ALLOC_ERROR, fatal: false, bytes: mdatSize, reason: `fail allocating video mdat ${mdatSize}` });
      return;
    }
      ...
      if (lastPTS !== undefined) {
        ...
        if (track.len > 0) {
          ...
          try {
            ...
            this.observer.trigger(Event.ERROR, { type: ErrorType.MUX_ERROR, details: ErrorDetail.REMUX_ALLOC_ERROR, fatal: false, bytes: mdatSize, reason: `fail allocating audio mdat ${mdatSize}` });
            return;
          }
          if (!rawMPEG) {
